...
class API:
    def __init__(self, host: str, timezone: str, version="v1"):
        self.api_base = f"http://{host}/api/flag/{version}"
    ...
    def info_flags(self, *flags: str):
        responses = list(
            map(lambda flag: requests.get(f"{self.api_base}/info/{flag}"), flags)
        )
    ...
    def submit_flags(self, *flags: str):
        ...
        responses = [
            requests.post(f"{self.api_base}/submit", data=flag, headers=h)
            for flag in flags
        ]
